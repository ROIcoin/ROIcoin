...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    if ((block.nVersion & FULL_FORK_VERSION_MAX) >= FULL_FORK_VERSION_MIN) {
        activePowLimit = Params().GetConsensus().powLimitResetAtFork;
    }
    if (!CheckProofOfWork(block.GetHash(), block.nBits, activePowLimit))
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck> *pvChecks)
{
	...
	// WORKAROUND FOR WINDOWS ROUNDING ERRORS
        if ((nValueIn+100) < tx.GetValueOut())
	...
	// WORKAROUND:: turn negative txfee into 0
        if (nTxFee < 0)
		nTxFee = 0;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
   ...
   if (block.GetHashNoCache() == chainparams.GetConsensus().hashGenesisBlock) { 
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.
                          !(pindex->nHeight>=1 && pindex->GetBlockHash() == uint256S("0x003852bfe1d708c4dfac4d48f5d678031c12f07fe478916c40351a84ab85747d"));
    if (fEnforceBIP30) {
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits, activePowLimit))
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < (chainActive.Height() < FORK5HEIGHT ? MIN_PEER_PROTO_FORK2_VERSION : MIN_PEER_PROTO_FORK5_VERSION))
        {
            // disconnect from peers older than this proto version
            LogPrintf("peer=%d using obsolete version %i; disconnecting\n", pfrom->id, pfrom->nVersion);
            pfrom->PushMessage("reject", strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be %d or greater", MIN_PEER_PROTO_FORK5_VERSION));
