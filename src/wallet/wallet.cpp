...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CROIcoinAddress(CScriptID(redeemScript)).ToString();
...
CAmount CWalletTx::GetDebit(const isminefilter& filter) const
{
    ...
    if(filter & ISMINE_SPENDABLE)
    {
        ...
        {
            nDebitCached = pwallet->GetDebit(*this, ISMINE_SPENDABLE, GetDepthInMainChain());
            fDebitCached = true;
            debit += nDebitCached;
        }
    }
    if(filter & ISMINE_WATCH_ONLY)
    {
        ...
        {
            nWatchDebitCached = pwallet->GetDebit(*this, ISMINE_WATCH_ONLY, GetDepthInMainChain());
...
CAmount CWalletTx::GetCredit(const isminefilter& filter, bool interest) const
{
    ...
    if (filter & ISMINE_SPENDABLE)
    {
        ...
        {
            nCreditCached = pwallet->GetCredit(*this, ISMINE_SPENDABLE, 0, interest);
            fCreditCached = true;
            credit += nCreditCached;
        }
    }
    if (filter & ISMINE_WATCH_ONLY)
    {
        ...
        {
            nWatchCreditCached = pwallet->GetCredit(*this, ISMINE_WATCH_ONLY, 0, interest);
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CROIcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CROIcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CROIcoinAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CROIcoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CROIcoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CROIcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CROIcoinAddress(dest).ToString(), key);
}
