...
{
    CROIcoinAddress addressParsed(address);
...
void static ROIcoinMiner(CWallet *pwallet, uint32_t minerI, uint32_t minerN, int nThreads)
{
    ...
    {
        LogPrintf("ROIcoinMiner terminated\n");
...
void GenerateROIcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    if(isInterrupting){
        return;
    }
    ...
    for(int i=0;i<miners;i++){
        minerThreads->create_thread(boost::bind(&ROIcoinMiner, pwallet, i, miners, nThreads));
        MilliSleep(200);
    }
}
