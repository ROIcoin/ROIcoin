...
void ROIcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a ROIcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ROIcoin: URIs)"));
...
/** Add new nav menu */
    ...
    aboutAction = new QAction(TextColorIcon(":/icons/about"), tr("About ROIcoin Core"), this);
    aboutAction->setStatusTip(tr("Show information about ROIcoin Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for ROIcoin Core"));
    ...
    openAction->setStatusTip(tr("Open a ROIcoin: URI or payment request"));

    showHelpMessageAction = new QAction(TextColorIcon(":/icons/info"), tr("Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the ROIcoin Core help message to get a list with possible ROIcoin command-line options"));
...
void ROIcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void ROIcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("ROIcoin Core client") + " " + networkStyle->getTitleAddText();
...
void ROIcoinGUI::optionsClicked()
{
...
void pwrcoinGUI::blockexplorer1ActionClicked()
{
        QDesktopServices::openUrl(QUrl("https://roi-coin-blockexplorer.roi-coin.com/"));
}

void pwrcoinGUI::blockexplorer2ActionClicked()
{
...
void pwrcoinGUI::facebookgroupActionClicked()
{
...
void pwrcoinGUI::googleplusActionClicked()
{
...
void pwrcoinGUI::telegramgroupActionClicked()
{
        QDesktopServices::openUrl(QUrl("https://t.me/joinchat/I0JIzQ72Fz6EWWAMwQURhQ"));
}

void pwrcoinGUI::telegramgannouncementsActionClicked()
{
...
void ROIcoinGUI::showHelpMessageClicked()
{
...
void ROIcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ROIcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ROIcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ROIcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ROIcoinGUI::gotoDepositCoinsPage(QString addr)
{
...
void ROIcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ROIcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void ROIcoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to ROIcoin network", "", count));
}

void ROIcoinGUI::setNumBlocks(int count, const QDateTime& blockDate)
{
    if(!clientModel)
        return;
...
{
    QString strTitle = tr("ROIcoin"); // default title
...
#ifdef ENABLE_WALLET
void ROIcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
...
void ROIcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ROIcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ROIcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ROIcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
#ifdef ENABLE_WALLET
void ROIcoinGUI::updateMiningIcon()
{
    if (fGenerate)
    {
...
void ROIcoinGUI::miningOff()
{
//mapArgs["-genproclimit"] = "0";
GenerateROIcoins(false, pwalletMain,0);
setMining(false,0, 0, 0);
}

void ROIcoinGUI::miningOn()
{
//mapArgs["-genproclimit"] = itostr(processes);
GenerateROIcoins(true, pwalletMain,-1);
...
void ROIcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
    ...
    menu(0)
{
    ...
    QList<ROIcoinUnits::Unit> units = ROIcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const ROIcoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(ROIcoinUnits::name(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu()
{
    ...
    {
        QAction *menuAction = new QAction(QString(ROIcoinUnits::name(u)), this);
...
{
    setText(ROIcoinUnits::name(newUnits));
}
