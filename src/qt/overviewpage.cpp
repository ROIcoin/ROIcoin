...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance, std::vector<COutput> termDepositInfo)
{
    ...
    // Fetch sort flag
    bool sort_flag = GetArg("-sortearnings", true);
    ...
    ui->ROITable->setRowCount(termDepositInfo.size());

    // actually update labels
    int nDisplayUnit = ROIcoinUnits::ROI;
    // Disable sorting outside the for loop 
    ui->ROITable->setSortingEnabled(false);
    ...
    for(int i=0;i<termDepositInfo.size();i++){
        ...
        if(!sort_flag){
            ui->ROITable->setItem(i, 6, new QTableWidgetItem(QString::number(lockHeight)));
            ui->ROITable->setItem(i, 7, new QTableWidgetItem(QString::number(releaseBlock)));
        }else{
            ui->ROITable->setItem(i, 6, new QTableWidgetItem(QString::number(lockHeight).rightJustified(7,'0')));
            ui->ROITable->setItem(i, 7, new QTableWidgetItem(QString::number(releaseBlock).rightJustified(7,'0')));
        }
        ...
        ui->ROITable->setItem(i, 8, new QTableWidgetItem(QString(buffer)));
    }
    ...
    if(sort_flag){
     ui->ROITable->setSortingEnabled(true);
    }
}
