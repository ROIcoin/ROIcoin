    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (about)
    {
        ...
        QString coreOptions = QString::fromStdString(HelpMessage(HMM_ROICOIN_QT));
        text = version + "\n" + header + "\n" + coreOptions;
...
void PaperWalletDialog::on_printButton_clicked()
{
    ...
    while (true) {
        ...
        // Ask for an amount to send to each paper wallet. It might be better to try to use the ROIcoinAmountField, but this works fine.
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("ROIcoin Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

void ShutdownWindow::showShutdownWindow(ROIcoinGUI *window)
{
    if (!window)
        return;
