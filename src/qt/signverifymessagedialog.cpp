    ...
    model(0)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::roicoinAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::roicoinAddressFont());
}
...
{
    CROIcoinAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CROIcoinAddress(pubkey.GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
