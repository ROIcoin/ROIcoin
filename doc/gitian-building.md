...
*Setup instructions for a Gitian build of ROIcoin using a Debian VM or physical system.*
...
- [Building ROIcoin](#building-roicoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-roicoin-host/README.md).
...
Clone the git repositories for roicoin and Gitian.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the roicoin repository
...
To build ROIcoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the roicoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/roicoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/roicoin/roicoin
...
For example:
```bash
URL=https://github.com/laanwj/roicoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit roicoin=${COMMIT} --url roicoin=${URL} ../roicoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit roicoin=${COMMIT} --url roicoin=${URL} ../roicoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit roicoin=${COMMIT} --url roicoin=${URL} ../roicoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/roicoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/roicoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/roicoin-osx-build.assert
```
...
[roicoin/gitian.sigs](https://github.com/roicoin/gitian.sigs/) repository, or if that's not possible create a pull
