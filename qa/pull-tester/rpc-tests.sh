...
export ROICOINCLI=${BUILDDIR}/qa/pull-tester/run-roicoin-cli
export ROICOIND=${REAL_ROICOIND}
...
if [ "x${ENABLE_ROICOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and roicoind must all be enabled"
fi
